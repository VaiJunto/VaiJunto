...
public class AdminManagementController {
 ...
 @PostMapping("/accounts") @PreAuthorize("hasRole('SUPER_ADMIN')") public Map<String,Object> createAccount(@RequestBody Map<String,String> body, Authentication auth) { return provisioning.create(auth.getName(),body.get("email"),body.get("role"),body.get("password")); }
 ...
 @PostMapping("/conversations/{id}/messages") public Map<String,UUID> sendAdminMessage(@PathVariable UUID id,@RequestBody Map<String,String> body,Authentication auth){return Map.of("conversationId",operations.sendAdminMessage(auth.getName(),id,body.get("body")));}
 @PostMapping("/users/{id}/newsletter") public void newsletter(@PathVariable UUID id,@RequestBody Map<String,String> body,Authentication auth){operations.newsletter(auth.getName(),id,body.get("title"),body.get("body"));}
