...
public class AdminManagementController {
 ...
 @PostMapping("/media") public Map<String,Object> uploadMedia(@RequestParam("file") org.springframework.web.multipart.MultipartFile file,@RequestParam(defaultValue="NEWSLETTER") String category,@RequestParam(required=false) Integer durationSeconds,Authentication auth) throws java.io.IOException {var id=media.uploadAdminMedia(auth.getName(),category,file.getContentType(),file.getSize(),durationSeconds,file.getBytes());return Map.of("mediaId",id,"url",media.adminMediaUrl(id));}
