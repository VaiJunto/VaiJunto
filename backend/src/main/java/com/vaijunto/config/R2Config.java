...
public class R2Config {
 ...
 @Bean public S3Client r2Client(R2Properties p){if(!p.configured())return null;return S3Client.builder().endpointOverride(URI.create(p.endpoint())).region(Region.of(p.region())).credentialsProvider(StaticCredentialsProvider.create(AwsBasicCredentials.create(p.accessKeyId(),p.secretAccessKey()))).serviceConfiguration(pathStyle()).build();}
 @Bean public S3Presigner r2Presigner(R2Properties p){if(!p.configured())return null;return S3Presigner.builder().endpointOverride(URI.create(p.endpoint())).region(Region.of(p.region())).credentialsProvider(StaticCredentialsProvider.create(AwsBasicCredentials.create(p.accessKeyId(),p.secretAccessKey()))).serviceConfiguration(pathStyle()).build();}
 private S3Configuration pathStyle(){return S3Configuration.builder().pathStyleAccessEnabled(true).build();}
